...
def query(expr: str, limit: int = 8192, format_type: str = "json") -> Dict[str, Any]:
    try:
        ...
        success, result = tools_manager.handle_tool_call(
            "query",
            {"expr": expr, "limit": limit, "format": format_type},
            context,
        )
...
def execute(code: str) -> str:
    try:
        context = _get_context()
        success, result = tools_manager.handle_tool_call("execute", {"code": code}, context)
...
def screenshot_object(object_name: str) -> Dict[str, Any]:
    try:
        context = _get_context()
        success, result = tools_manager.handle_tool_call(
            "screenshot_object",
