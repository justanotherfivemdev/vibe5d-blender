...
class MarkdownMessageComponent(UIComponent):
        ...
        for element in self.elements:
            ...
            if element.element_type == 'image' and element.url:
                ...
                try:
                    ...
                    logger.info(
                        f"MarkdownMessage: Creating code block component - language='{element.code_language}', width={available_width}")
                    ...
                    logger.info(
                        f"MarkdownMessage: Code block component created - bounds={element.code_block_component.bounds}")
                    logger.debug(
                        f"Created code block component: {len(element.text.split())} words, language='{element.code_language}'")
                except Exception as e:
            ...
            if element.image_component:
                ...
                if hasattr(element.image_component, 'state'):
                    if element.image_component.state == URLImageState.LOADED:
                        ...
                        logger.debug(
                            f"Loading state height: {loading_height} (container: {container_width}, max: {max_height})")
    ...
    def _wrap_formatted_text(self, element, available_width: int) -> List[tuple]:
        ...
        for line_spans in lines:
            ...
            for mini in line_spans:
                if current_part and current_part['type'] == mini['type'] and current_part['fs'] == mini['fs'] and \
                        current_part['color'] == mini['color'] and current_part['font_id'] == mini['font_id']:
    ...
    def set_size(self, width: int, height: int):
            ...
            for element in self.elements:
                if element.element_type == 'image' and element.image_component:
                    ...
                    if hasattr(element.image_component, 'set_container_width'):
                        ...
                        logger.debug(
                            f"Updating URL image container width from {element.image_component.container_width} to {available_width}")
                        element.image_component.set_container_width(available_width)
                    else:
        ...
        if self.on_height_changed and old_height != height:
            try:
                logger.debug(
                    f"Triggering parent layout update due to programmatic size change: {old_height} -> {height}")
            ...
            if element.element_type == 'block':
                ...
                if current_y >= y and element.image_component:
                    ...
                    if not hasattr(element.image_component, 'state'):
                        ...
                        logger.debug(
                            f"Rendering URL image at position ({x}, {current_y}) with size ({element.image_component.bounds.width}, {element.image_component.bounds.height})")

                    element.image_component.render(renderer)

                    self._store_text_position(element_index, element.alt_text or element.url, x, current_y, width,
                                              line_height)
                element_index += 1
            elif element.element_type == 'table':
    ...
    def _calculate_table_column_widths(self, table_element, available_width: int) -> List[int]:
        ...
        for col_index in range(num_columns):
            ...
            if col_index < len(table_element.table_headers):
                ...
                header_width = \
                self._get_cached_text_dimensions(header_text, table_element.font_size, table_element.font_id)[0]
            ...
            for row_data in table_element.table_rows:
                if col_index < len(row_data):
                    ...
                    cell_width = \
                    self._get_cached_text_dimensions(cell_text, table_element.font_size, table_element.font_id)[0]
