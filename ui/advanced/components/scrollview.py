...
class ScrollView(UIComponent):
    ...
    def _update_content_bounds(self):
        ...
        if not self.children:
            ...
            self.max_scroll_y = 0
            logger.debug(
                f"ScrollView no children: content_bounds={self.content_bounds}, max_scroll_y={self.max_scroll_y}")
            return
        ...
        logger.debug(
            f"ScrollView content updated: children={len(self.children)}, content_bounds={self.content_bounds}, scrollview_bounds={self.bounds}")
    ...
    def _update_scroll_limits(self):
        ...
        logger.debug(
            f"ScrollView scroll limits: max_scroll_x={self.max_scroll_x}, max_scroll_y={self.max_scroll_y}, content_size=({self.content_bounds.width}, {self.content_bounds.height}), available_size=({available_width}, {available_height})")
    ...
    def _on_mouse_wheel(self, event: UIEvent) -> bool:
        ...
        if self.scroll_direction in [ScrollDirection.VERTICAL, ScrollDirection.BOTH]:
            ...
            if old_scroll_y != self.scroll_y:
                ...
                logger.debug(
                    f"ScrollView vertical scroll: {old_scroll_y} -> {self.scroll_y} (reverse: {self.reverse_y_coordinate}, max: {self.max_scroll_y})")
                return True
            else:
