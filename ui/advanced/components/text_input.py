...
class TextInput(UIComponent):
    ...
    def render(self, renderer: 'UIRenderer'):
        ...
        if self.is_scrollable or self.is_horizontally_scrollable:
            ...
            if self.multiline:
                ...
                renderer.push_clip_rect(content_x, content_y - CoordinateSystem.scale_int(8), content_width,
                                        self.bounds.height + CoordinateSystem.scale_int(16))
        ...
        if self.text:
            ...
            self._render_placeholder_wrapped(renderer, content_x, content_y, content_width, line_height,
                                             placeholder_color)
    ...
    def _render_text_content_with_scroll(self, renderer: 'UIRenderer', start_x: int, start_y: int, width: int,
                                         line_height: int):
    ...
    def _render_single_line_text(self, renderer: 'UIRenderer', start_x: int, start_y: int, width: int,
                                 line_height: int):
    ...
    def _render_placeholder_wrapped(self, renderer: 'UIRenderer', start_x: int, start_y: int, width: int,
                                    line_height: int, placeholder_color: Tuple[float, float, float, float]):
    ...
    def _render_single_line_placeholder(self, renderer: 'UIRenderer', start_x: int, start_y: int,
                                        placeholder_color: Tuple[float, float, float, float]):
    ...
    def _validate_cursor_position(self):
        ...
        if self.cursor_row < len(self._text_lines):
            ...
            if self.cursor_col < 0:
                ...
                logger.warning(
                    f"Cursor col {self.cursor_col} is out of bounds for row {self.cursor_row}, clamping to {max_col}")
                self.cursor_col = max_col
                cursor_changed = True
    ...
    def _render_single_line_selection(self, renderer: 'UIRenderer', content_x: int, content_y: int, line_height: int,
    ...
    def _scroll_horizontally_by(self, delta: int):
        ...
        self.horizontal_scroll_offset = max(0, min(self.horizontal_scroll_offset + delta,
    ...
    def _update_horizontal_scroll_state(self):
        ...
        if text_width > available_width:
            ...
            self.horizontal_scroll_offset = max(0,
                                                min(self.horizontal_scroll_offset, self.max_horizontal_scroll_offset))
        else:
            self.is_horizontally_scrollable = False
            self.horizontal_scroll_offset = 0
            self.max_horizontal_scroll_offset = 0
    ...
    def _draw_cursor(self, renderer: 'UIRenderer', content_x: int, content_y: int, content_width: int,
                     content_height: int):
