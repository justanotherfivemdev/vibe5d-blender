...
class UIManager:
    ...
    def _handle_send(self):
        ...
        try:
            ...
            self._current_ai_component = self.factory.add_markdown_message_to_scrollview(self.components, "",
                                                                                         is_ai_response=True)
        ...
        def update_ui():
            try:
                ...
                if hasattr(response, 'current_tool_status') and hasattr(response, 'current_tool_name'):
                    ...
                    if tool_status == "started" and tool_name:
                        ...
                        call_id = getattr(response, 'current_tool_call_id',
                                          f"call_{len(self._conversation_tracking['tool_calls'])}")

                        logger.info(
                            f"Tool start: call_id='{call_id}', already_created={call_id in self._tool_start_blocks_created}, created_set={self._tool_start_blocks_created}")
                        ...
                        call_id = getattr(response, 'current_tool_call_id',
                                          f"call_{len(self._conversation_tracking['tool_responses'])}")
                        ...
                        if completion_block:
                            ...
                            if self._update_tool_component(call_id, completion_block):
                                ...
                                if call_id not in self._tool_completion_blocks_created:
                                    ...
                                    self._tool_completion_blocks_created.add(call_id)
                                    logger.info(
                                        f"Marked call_id '{call_id}' as having completion block. Set now: {self._tool_completion_blocks_created}")
                ...
                if hasattr(response, 'output_content') and response.output_content:
                    ...
                    logger.debug(
                        f"Content length: {len(content)}, Last tracked: {self._last_content_length}, Content after tool call: {self._content_after_tool_call}")
                    ...
                    if self._content_after_tool_call and len(content) > self._last_content_length:
                        ...
                        logger.debug(
                            f"Streaming update: full_content='{content[:50]}...', new_content='{new_content[:20]}...', last_length={self._last_content_length}")
                        ...
                        if current_content.strip() and not current_content.startswith('['):
                            ...
                            logger.debug(
                                f"Appending to existing message: '{new_content[:30]}...' to '{current_content[:30]}...'")
                            ...
                            if current_content.endswith(']'):
                                ...
                                updated_content = current_content + new_content
                                logger.debug(
                                    f"Appending to completed status block: '{new_content[:30]}...' to '{current_content[:30]}...'")
                            else:
                ...
                if hasattr(response, 'output_content') and response.output_content:
                    ...
                    if self._current_ai_component:
                        ...
                        if current_content != final_content:
                            logger.info(
                                f"Finalizing existing AI component with complete content: '{final_content[:50]}...'")
                            if hasattr(self._current_ai_component, 'set_markdown'):
                        ...
                        if new_content:
                            logger.info(
                                f"No existing AI component, creating new message for final content: '{new_content[:50]}...'")
                            ...
                            if self._content_after_tool_call:
                                ...
                                logger.info(
                                    "No current AI component and not in post-tool-call mode, content may have been lost")
                        else:
                ...
                if self._conversation_tracking and not self._conversation_tracking['conversation_saved']:
                    try:
                        ...
                        if final_content or tool_calls:
                            ...
                            logger.info(
                                f"✓ Saved partial ASSISTANT message on completion error: '{(final_content or '')[:50]}...' with {len(tool_calls)} tool calls")
                        ...
                        if tool_responses:
                            ...
                            for tool_response in sorted_tool_responses:
                                simple_response_text = tool_response.get("ui_message") or tool_response.get("content",
                                                                                                            "Tool response")
                                history_manager.add_message(
                                    context,
                                    "tool",
                                ...
                                logger.debug(
                                    f"✓ Saved TOOL response on completion error for call_id: {tool_response.get('call_id')}")
                ...
                if self._conversation_tracking and not self._conversation_tracking['conversation_saved']:
                    try:
                        ...
                        if final_content or tool_calls:
                            ...
                            logger.info(
                                f"✓ Saved partial ASSISTANT message on error: '{(final_content or '')[:50]}...' with {len(tool_calls)} tool calls")
    ...
    def _track_tool_call(self, tool_call_data: dict):
        ...
        logger.debug(
            f"Tracked tool call: {tool_call_data.get('function', {}).get('name', 'unknown')} (call_id: {call_id})")
    ...
    def _save_conversation_to_history(self):
        ...
        try:
            ...
            if final_content or tool_calls:
                ...
                logger.info(
                    f"✓ Saved ASSISTANT message: '{(final_content or '')[:50]}...' with {len(tool_calls) if tool_calls else 0} tool calls")
            ...
            if self._conversation_tracking['tool_responses']:
                ...
                for tool_response in sorted_tool_responses:
                    ...
                    logger.debug(
                        f"✓ Saved TOOL response for call_id: {tool_response.get('call_id')} as simple text: '{simple_response_text}'")
        ...
        try:
            ...
            if (self.state.viewport_width != new_width or
                ...
                if is_initial_draw:
                    ...
                    viewport_changed = True
                    logger.debug(
                        f"Viewport resizing: {self.state.viewport_width}x{self.state.viewport_height} -> {new_width}x{new_height}")
    ...
    def handle_external_tool_response(self, tool_response_data: dict):
        ...
        try:
            ...
            logger.info(
                f"🔧 Tracked external tool response for call_id: {call_id} (history: original result, UI: '{ui_message}')")
