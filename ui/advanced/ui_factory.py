...
class ImprovedUIFactory:
    ...
    def add_message_to_scrollview(self, components, text: str, is_ai_response: bool = False):
        ...
        if self.current_view == ViewState.MAIN:
            ...
            logger.info(
                f"Added MessageComponent to scrollview: {text} (size: {message_component.bounds.width}x{message_component.bounds.height}, total messages: {len(message_scrollview.children)})")
    ...
    def add_markdown_message_to_scrollview(self, components, markdown_text: str, is_ai_response: bool = False):
        ...
        if self.current_view == ViewState.MAIN:
            ...
            logger.info(
                f"Added MarkdownMessageComponent to scrollview: {markdown_text[:50]}... (size: {message_component.bounds.width}x{message_component.bounds.height}, total messages: {len(message_scrollview.children)})")
    ...
    def add_image_message_to_scrollview(self, components, text: str, image_data_uri: str, is_ai_response: bool = False):
        ...
        if self.current_view == ViewState.MAIN:
            ...
            logger.info(
                f"Added image message text to scrollview (image disabled): {text} (size: {text_component.bounds.width}x{text_component.bounds.height})")
    ...
    def add_error_message_to_scrollview(self, components, error_text: str):
        ...
        if self.current_view == ViewState.MAIN:
            ...
            logger.info(
                f"Added ErrorMessageComponent to scrollview: {error_text[:50]}... (size: {error_component.bounds.width}x{error_component.bounds.height}, total messages: {len(message_scrollview.children)})")
